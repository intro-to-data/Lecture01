...
For example, `data/clean_data.R`.

- Script I wrote to remove identifying information from the class survey and make it easier to work with.
- **Note:** The raw class survey document is not part of what I will share with the class.
...
```{r}
survey$tried_use_data
```

- This kind of free text entry is a special case.
- There are categories here. For example, I could group these answers into:
    - Yes, No, Unknown, NA
- But there is an important difference between this and the other categoricals we have evaluated.
  -  YOU would not choose the group.
  -  As arbitrary as this survey is - you had the power to group yourself.
  -  In this case, the researcher (me!) would have that power.
  -  And this can be a powerful way to explore a small data set. This is often done early in a research process. But it is always important to know when the respondents choose the group and when someone else chooses the group based on free text entries.
...
survey |>
  ...
  summarise(n = n()) |>
  pivot_wider(names_from = "ever_analyzed_data", values_from = n)
```
...
```{r}
ggplot(survey, aes(x = what_state)) +
  geom_bar()
```
